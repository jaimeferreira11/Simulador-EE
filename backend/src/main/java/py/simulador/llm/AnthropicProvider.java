...
@Component
@ConditionalOnProperty(name = "simulador.llm.provider", havingValue = "anthropic")
public class AnthropicProvider implements LlmProvider {
    ...
    public AnthropicProvider(LlmProperties props) {
        this.client = RestClient.builder()
                .baseUrl("https://api.anthropic.com/v1")
                .defaultHeader("x-api-key", props.apiKey())
                .defaultHeader("anthropic-version", "2023-06-01")
                .build();
        this.model = props.model() != null ? props.model() : "claude-haiku-4-5-20251001";
    ...
    public LlmCompletion completarPrompt(String prompt) {
        return callRich(prompt, 800);
    }

    @SuppressWarnings("unchecked")
    private String call(String prompt) {
        return callRich(prompt, 400).content();
    }

    @SuppressWarnings("unchecked")
    private LlmCompletion callRich(String prompt, int maxTokens) {
        var body = Map.of(
            "model", model,
        ...
        if (usage != null) {
            Object inT = usage.get("input_tokens");
            Object outT = usage.get("output_tokens");
            if (inT instanceof Number n) promptTokens = n.intValue();
            if (outT instanceof Number n) completionTokens = n.intValue();
        }
